                            ...
                            {orderDetails.orderItems.map((it) => (
                                <tr key={it.productId} className="border-b">
                                    <td className="px-4 py-2 flex items-center">
                                       <img src={it.image} alt={it.name} className="w-12 h-12 object-cover rounded-lg mr-4"/> 
                                       <Link to={`/product/${it.productId}`} className="text-blue-600 hover:underline">
                                            {it.name}
                                       </Link>
                                    </td>
                                    <td className="px-4 py-2">₦{it.price}</td>
                                    <td className="px-4 py-2">{it.quantity}</td>
                                    <td className="px-4 py-2 font-bold">₦{it.price * it.quantity}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
